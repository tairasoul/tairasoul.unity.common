...
class CompilationVisitor : IVisitor {
	...
	static ConstructorInfo BoundsPtrWrapper = typeof(BoundsPtrWrapper).GetConstructor([typeof(Bounds*)]);
	static MethodInfo AccessorUtilGet = typeof(AccessorUtil).GetMethod("Get", BindingFlags.Public | BindingFlags.Static, null, [typeof(object), typeof(Type), typeof(string)], []);
	...
	public void Visit(TranslationBoundsGrouped grouped) {
		...
		if (grouped.grouped is GroupedBoundType.Once or GroupedBoundType.Never) {
			...
			sl.Emit(OpCodes.Ldarg_0);
			sl.Emit(OpCodes.Ldflda, boundField);
			sl.Emit(OpCodes.Newobj, BoundsPtrWrapper);
			...
			gen.Emit(OpCodes.Ldarg_0);
			gen.Emit(OpCodes.Ldflda, boundField);
			gen.Emit(OpCodes.Newobj, BoundsPtrWrapper);
			gen.Emit(OpCodes.Ldarg_0);
			gen.Emit(OpCodes.Ldc_I4_0);
			...
			os.Emit(OpCodes.Ldarg_0);
			os.Emit(OpCodes.Ldflda, boundField);
			os.Emit(OpCodes.Newobj, BoundsPtrWrapper);
			...
			currentGenerator.Emit(OpCodes.Ldarg_0);
			currentGenerator.Emit(OpCodes.Ldflda, boundField);
			currentGenerator.Emit(OpCodes.Newobj, BoundsPtrWrapper);
